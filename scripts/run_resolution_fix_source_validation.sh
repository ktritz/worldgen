...
OUT_ROOT="$ROOT_DIR/output/review_planets/sweeps/resolution_fix_source_validation"
SEEDS="4,42,84,91,123,255,314,777"
GO_BIN="/usr/local/go/bin/go"
GOCACHE_DIR="/tmp/go-build-cache"

REVIEW_ARGS=(
  --review-arg=-climate-settlement-network=false
  --review-arg=-climate-proto-civilizations=false
  --review-arg=-climate-trade-network=false
  --review-arg=-climate-river-trade=false
  --review-arg=-climate-coastal-ports=false
  --review-arg=-climate-coastal-trade=false
  --review-arg=-climate-ocean-trade=false
  --review-arg=-climate-polity-spheres=false
)
...

{
  ...
  echo "seeds=$SEEDS"
  echo "purpose=resolution_fix_source_validation"
  echo "disabled_phases=settlement_network,proto_civilizations,trade_network,river_trade,coastal_ports,coastal_trade,ocean_trade,polity_spheres"
