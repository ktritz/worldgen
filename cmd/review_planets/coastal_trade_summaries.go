...
func printCoastalTradeSummary(result *climgen.CoastalTradeResult, network *climgen.SettlementNetworkResult) {
	...
	if len(result.Corridors) == 0 {
		...
		fmt.Printf(
			"      endpointDiag: endpoints=%d ports=%d stopovers=%d edges=%d components=%d largest=%d largestPorts=%d isolatedPorts=%d\n",
		...
		if result.PairDiagnostics.TotalPairs > 0 {
			fmt.Printf(
				"      pairDiag: total=%d viable=%d noPath=%d flowBelow=%d portCap=%d civCap=%d bestRejectedFlow=%.2f bestRejectedCost=%.2f pair=%d->%d\n",
	...
	fmt.Printf(
		"      pairDiag: total=%d viable=%d noPath=%d flowBelow=%d portCap=%d civCap=%d\n",
		result.PairDiagnostics.TotalPairs,
		result.PairDiagnostics.ViableCandidates,
		result.PairDiagnostics.NoPath,
		result.PairDiagnostics.FlowBelowMin,
