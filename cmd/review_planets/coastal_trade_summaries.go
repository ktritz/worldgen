...
func printCoastalTradeSummary(
	...
	if len(result.Corridors) == 0 {
		fmt.Printf(
			"    coastalTrade[%s]: corridors=0 candidatePorts=%d stopovers=%d majorPorts=%d coastLength=%.6f oceanArea=%.6f portsPerCoastLength=%.6f coastalCorridorsPerCoastLength=%.6f stopoversPerOceanArea=%.6f\n",
			result.Mode.Name,
	...
	fmt.Printf(
		"    coastalTrade[%s]: corridors=%d candidatePorts=%d stopovers=%d majorPorts=%d inter=%d meanFlow=%.2f meanExposure=%.2f meanAssist=%.2f coastLength=%.6f oceanArea=%.6f portsPerCoastLength=%.6f coastalCorridorsPerCoastLength=%.6f stopoversPerOceanArea=%.6f\n",
		result.Mode.Name,
		...
		denominators.OceanArea,
		climgen.PerExtent(len(result.CandidatePorts), denominators.CoastLength),
		climgen.PerExtent(len(result.Corridors), denominators.CoastLength),
		climgen.PerExtent(len(result.Stopovers), denominators.OceanArea),
	)
	fmt.Printf(
